...
import sys
```
###### import로 파이게임 라이브러리, 랜덤 모듈, 시스템 모듈을 불러 온다.
...
```
###### pygame.init()으로 pygame 모듈을 초기화한다. 게임 시작 전 반드시 필요하다.
###### WIDTH와 HEIGHT으로 게임 화면 크기를 설정한다. 600x400 픽셀로 지정했다.
###### pygame.display.set_mode()으로 게임 창을 생성한다.
###### set_caption()으로 게임 창의 제목을 설정한다.
...
BLACK = (0, 0, 0)
```
###### RGB 컬러 코드를 사용해 게임에 필요한 기본 색상을 정의한다.
###### 배경은 검정색, 뱀은 초록색, 음식은 빨간색으로 설정될 것이다.
...
food = (random.randrange(0, WIDTH, block_size), random.randrange
(0, HEIGHT, block_size))
```
###### 뱀 몸통 크기를 block_size으로 지정하고, snake에 뱀의 위치를 리스트로 저장한다.
###### 뱀은 각 블록 (x, y) 좌표로 표현되며 초기 위치는 세 블록으로 구성된다.

###### 일반적으로 Pygame에서 (0, 0)은 화면의 왼쪽 위 모서리이다.
###### y가 증가하면 아래쪽, 감소하면 위쪽으로 이동한다.
###### x가 증가하면 오른쪽, 감소하면 왼쪽으로 이동한다.

###### snake_dir으로 뱀의 이동 방향을 나타낸다. (block_size, 0)은 오른쪽 이동으로, 초기에는 오른쪽으로 이동한다.

###### EX: (block_size, 0)는 오른쪽으로 이동
######     (0, block_size)는 아래쪽으로 이동
######     (0, -block_size)는 위쪽으로 이동
######     (-block_size, 0)는 왼쪽으로 이동

```python
clock = pygame.time.Clock()
```
###### 게임 프레임 속도(FPS)를 제어하는데 사용된다. 게임 루프가 극단적으로 실행되면 뱀이 지나치게 빠르게 움직여서 플레이에 지장이 생기거나, 너무 느리다면 답답한 플레이 경험을 하게 된다.
###### clock 객체를 통해 매 프레임마다 일정한 시간 간격으로 루프가 실행되도록 조정할 수 있다.
...
while True:
    screen.fill(BLACK)
```
###### while True 루프로 게임이 계속 실행되게 유지한다.
###### screen.fill(BLACK)으로 화면을 검정색으로 채운다. 게임 루프가 반복될 때마다 이전 프레임의 이미지를 지워 뱀이 이동한 경로를 화면에 남지 않게 한다.
...
for event in pygame.event.get():
    if event.type == pygame.QUIT:
        ...
        sys.exit()
```
###### pygame.event.get()으로 사용자 입력을 처리한다.
###### EX: 키보드, 마우스, 창 닫기 등
######     사용자가 창 닫튼 이벤트을 발생시키면 pygame.quit()와 sys.exit()을 통해 게임을 종료한다.
...
![tailCut](https://github.com/user-attachments/assets/47012614-811a-4410-b3a3-459c5042c022)
